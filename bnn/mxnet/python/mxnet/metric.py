    ...
    def get(self):
        ...
        for metric in self.metrics:
            ...
            if isinstance(name, string_types):
                name = [name]
            if isinstance(value, (numeric_types,numpy.ndarray)):
                value = [value]
            names.extend(name)
            values.extend(value)
        return (names, values)
...
@register
class Conf_Mat(EvalMetric):
    def __init__(self, name='conf_mat',output_names=None, label_names=None):
        self.metrics=_BinaryClassificationMetrics()
        EvalMetric.__init__(self, name=name, output_names=output_names, label_names=label_names)
    ...
    def update(self, labels, preds):
        ...
        # self.num_inst=2
        self.confmat=numpy.array([[self.metrics.true_negatives, self.metrics.false_negatives],[self.metrics.false_positives, self.metrics.true_positives]])
        self.recall=self.metrics.recall
	
    def reset(self):
        self.num_inst=0
        self.confmat=numpy.array([[0,0],[0,0]])
        self.recall=0.
    ...
    def get(self):
        ...
        values : list of float
           Value of the evaluations.
        """
        return (['conf_mat','recall'], [self.confmat,self.recall])


@register
class Recall(EvalMetric):
    def __init__(self, name='recall',output_names=None, label_names=None):
        self.metrics=_BinaryClassificationMetrics()
        EvalMetric.__init__(self, name=name, output_names=output_names, label_names=label_names)
    ...
    def update(self, labels, preds):
        ...
        self.num_inst=self.metrics.total_examples
        # self.num_inst=2
        self.sum_metric=self.metrics.recall
    def reset(self):
        self.num_inst=0
        self.sum_metric=0.
        self.metrics.reset_stats()
