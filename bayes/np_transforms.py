...
class RandomCrop(object):
    ...
    @staticmethod
    def get_params(pic, output_size, padding):
        ...
        # get crop indexes
        i = random.randint(0, w - tw + padding*2)
        j = random.randint(0, h - th + padding*2)
    ...
    def __call__(self, pic):
        ...
        i, j, th, tw = self.get_params(pic, self.size, self.padding)

        if self.padding:
            pic1 = np.pad(pic, [(self.padding,self.padding),(self.padding,self.padding),(0,0)])

        # perform cropping and return the new image
        return pic1[i:i + th, j:j + tw, :]
...
def aug_data_binary(tr, te):
    trlen = tr.ft_buffer.shape[0]
    telen = te.ft_buffer.shape[0]
    # ratio0 = telen/trlen
    # ratio = 1/(1+pow(ratio0,-2))
    ratio0 = np.random.random_sample()*0.3+0.3
    ratio1 = np.random.random_sample()*0.3+0.3
    # print(ratio0, ratio1)
    trones = tr.label_buffer.sum()
    trzeros = len(tr.label_buffer)-trones
    teones = te.label_buffer.sum()
    tezeros = len(te.label_buffer)-teones
    teoneindex = np.arange(telen)[te.label_buffer==1]
    tezeroindex = np.arange(telen)[te.label_buffer==0]
    troneindex = np.arange(trlen)[tr.label_buffer==1]
    trzeroindex = np.arange(trlen)[tr.label_buffer==0]    
    tmp = np.random.choice(teoneindex, int(ratio1*trones), replace=True)
    trone = np.concatenate((tr.ft_buffer[troneindex][:(trones-int(ratio1*trones))], te.ft_buffer[tmp]), axis=0)
    tronel = np.concatenate((tr.label_buffer[troneindex][:(trones-int(ratio1*trones))], te.label_buffer[tmp]), axis=0)

    tmp = np.random.choice(tezeroindex, int(ratio0*trzeros), replace=True)
    trzero = np.concatenate((tr.ft_buffer[trzeroindex][:(trzeros-int(ratio0*trzeros))], te.ft_buffer[tmp]), axis=0)
    trzerol = np.concatenate((tr.label_buffer[trzeroindex][:(trzeros-int(ratio0*trzeros))], te.label_buffer[tmp]), axis=0)
    tr.ft_buffer = np.concatenate((trone, trzero), axis=0)
    tr.label_buffer = np.concatenate((tronel, trzerol), axis=0)
